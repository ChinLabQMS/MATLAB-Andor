...
% Initialize matrices for trap depth and trap frequency results
...
for i = 1:length(w0_values)
    ...
    for j = 1:length(P_values)
        ...
        % Calculate trap frequency based on intensity maximum curvature
        ...
        if x_max_idx > 1 && x_max_idx < length(x_range) % ensure the index is within bounds
            ...
            % Calculate the trap frequency and store in matrix
...
hold on;
levels = [1.01,1.001]; % Define specific levels for trap_frequency_ratios
...
trap_frequency_levels = [200000,210000,220000]; % Example trap frequency levels in Hz
