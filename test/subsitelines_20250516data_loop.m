...
folder = "data/2025/05 May/20250516/";
fileList = dir(fullfile(folder,'vec2_width5_offset=*.mat'));
%filename = "vec2_width5_offset=5.mat";
% Find the index of the file with offset=11
fileList1 = {fileList.name};
matchIdx = find(contains(fileList1(:,1), 'offset=11'));
matchIdx = 5;
% Remove it from its current position
fileToMove = fileList1(matchIdx);
fileList1(matchIdx) = [];
% Append it to the end
fileList1{end+1} = fileToMove{1}
%%
...
%for i = 1:2
    ...
    Data = load(fullfile(folder,fileList1{i})).Data;
...
%% convert offsets into projected distance in DMD pixels
projdist = Sqrt[2]
